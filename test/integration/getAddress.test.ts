  ...
  if (REGISTRY_CONTRACT_ADDRESS) {
    ...
    describe('.getAddress()', () => {
      ...
      it('expect to throw when name to long', async () => {
        await expectThrowsAsync(() => registry.getAddress('a'.repeat(33)), Error, 'name is to long, max supported length is 32 bytes, got: 33');
      });
    ...
    console.warn('skipping Registry integration tests');
  }
});
